...
st.set_page_config(page_title="Plan to Plate", page_icon='icon.png')
if "Chat_history" not in st.session_state:
    st.session_state["Chat_history"] = []
with open('style.css') as f:
    st.markdown(f"<style>{f.read()}</style>", unsafe_allow_html=True)
st.sidebar.success("Select a page")
st.title("Welcome 👋 to Plan to Plate!")
st.write('----')
st.image("pic.png")
st.subheader("Build Your Favourite Meal Plan, Right Now!😉")
st.write('----')
input_prompt = """
You are expert in Cooking so whenever user write a list of ingredients names, tell the user 3 examples of food can be made using input ingredients how to cook the meal mentioning the 
ingredients amounts to cook the meal and instructions to cook the meal and also mention which nutrients the meal have."
In the below format:
                Mutton Kosha
Ingredients:
...
input_text = st.text_area("**Enter a Comma-Separated List of Ingredients**", key='input_text')
...
if len(input_text) > 0 and submit:
    with st.spinner("**Crafting Your Beloved Recipe...😋**"):
        ...
        st.subheader("Let's get creative in the kitchen!🧑‍🍳")
        st.write(response)
    ...
    st.write("**Write Ingredients to Cook a New Recipe! **")
